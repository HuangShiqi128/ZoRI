...
from .modeling.transformer_decoder.zori_transformer_decoder import MaskPooling, get_classification_logits, get_classification_logits_cache
...
class ZORI(nn.Module):
    ...
    def get_text_classifier(self):
        if self.training:
            if self.train_text_classifier is None:
                ...
                text_classifier = text_classifier.reshape(text_classifier.shape[0]//len(RESISC45_PROMPT), len(RESISC45_PROMPT), text_classifier.shape[-1]).mean(1)
                text_classifier /= text_classifier.norm(dim=-1, keepdim=True)
    ...
    def from_config(cls, cfg):
        ...
        return {
            ...
            "geometric_ensemble_alpha": cfg.MODEL.ZORI.GEOMETRIC_ENSEMBLE_ALPHA,
            "geometric_ensemble_beta": cfg.MODEL.ZORI.GEOMETRIC_ENSEMBLE_BETA,
            "ensemble_on_valid_mask": cfg.MODEL.ZORI.ENSEMBLE_ON_VALID_MASK
        }
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            if self.cache:
                out_vocab_cls_results = get_classification_logits_cache(self, pooled_clip_feature, text_classifier, self.backbone.clip_model.logit_scale, num_templates)
            else:
            ...
            for mask_cls_result, mask_pred_result, input_per_image, image_size in zip(
                ...
                if self.instance_on:
                    if self.generalized:
                        ...
                        instance_r = retry_if_cuda_oom(self.instance_inference_zsis)(mask_cls_result, mask_pred_result)
    ...
    def instance_inference_zsis(self, mask_cls, mask_pred):
            # mask_pred is already processed to have the same shape as original input
            image_size = mask_pred.shape[-2:]
            self.num_unseen = len(self.test_metadata.thing_classes)

            mask_cls = mask_cls[:, -(self.num_unseen+1):]
            scores = F.softmax(0.1*mask_cls, dim=-1)[:, :-1]
            labels = torch.arange(self.num_unseen, device=self.device).unsqueeze(0).repeat(self.num_queries, 1).flatten(0, 1)

            scores_per_image, topk_indices = scores.flatten(0, 1).topk(self.test_topk_per_image, sorted=False)
            labels_per_image = labels[topk_indices]

            topk_indices = topk_indices // self.num_unseen
            mask_pred = mask_pred[topk_indices]
            # if this is panoptic segmentation, we only keep the "thing" classes
            if self.panoptic_on:
                keep = torch.zeros_like(scores_per_image).bool()
