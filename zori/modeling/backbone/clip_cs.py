...
class CLIP_CS(Backbone):
    def __init__(self, cfg, input_shape):
        ...
        all_indices = torch.arange(192)
        top_indices_set = set(self.indices.tolist())  # Convert top-k indices to a set for faster lookup
        self.rest_indices = [idx for idx in all_indices.tolist() if idx not in top_indices_set]\
        
        stem_layer_conv2d = self.clip_model.visual.trunk.stem[0]
        out_channel, in_channel, kernel_size, stride = stem_layer_conv2d.out_channels, stem_layer_conv2d.in_channels, stem_layer_conv2d.kernel_size, stem_layer_conv2d.stride
        ...
        #self.adapter = MLP(self._out_feature_channels['stem'], self._out_feature_channels['stem'], self._out_feature_channels['stem'], 2)
        # self.eval()
    ...
    def extract_features_convnext(self, x):
        ...
        x = torch.zeros_like(x_train)
        x[:, self.rest_indices, :, :] = x_train[:, self.rest_indices, :, :]
        x[:, self.indices.tolist(), :, :] = x_freeze[:, self.indices.tolist(), :, :]
